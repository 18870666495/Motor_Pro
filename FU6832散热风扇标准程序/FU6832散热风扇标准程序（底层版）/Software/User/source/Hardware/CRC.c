...
/* Private variables ---------------------------------------------------------*/
code const uint8 crc8_Table[256]  = {
...
/**
  * @brief      CRC8 軟體計算函數
  * @note       本函數僅適用於 HPFCOL 功能內
  * @param[in]  uint8 ptr[] 輸入資料
  * @param[in]  uint8 len   資料長度
  * @retval uint8_t crc 
  */
uint8 SW_CRC8_Calc(uint8 ptr[], uint8 len)
{
  uint8 crc = 0x00;
  uint8 cnt = 0;
  while (len--)
  {
    crc = crc8_Table[crc ^ ptr[cnt++]];
  }
  return (crc);
}
