...
volatile unsigned short Temp_DAC0 = 0x00;
 ...
 *     00-->P27-单比较器模式    
 *     01-->P20/P23-双比较器模式
 *     1X-->P20/P23/P27-三比较器模式
*****************************************************************************/

void CMP3_Init(void)
{
		uint16 DAC_OvercurrentTempValue;
    /******CMP3 端口模拟功能设置*******/    
    ...
    #elif (Compare_Mode == Compare_DAC)
    {
        ...
        ****************************/
        ...
        /**********设置DAC过流值*****************/
		DAC_OvercurrentTempValue = DAC_OvercurrentValue;
        if(DAC_OvercurrentTempValue % 2 == 0)
        {
...
//        DAC0_DR = DAC_OvercurrentValue;
				DAC0_DR   = DAC_OvercurrentValue >> 1;
        Temp_DAC0 = DAC_OvercurrentValue;
